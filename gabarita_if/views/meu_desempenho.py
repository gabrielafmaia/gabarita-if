...
def meu_desempenho(request):
    ...
    total_respostas = RespostaQuestao.objects.filter(usuario=usuario).count()
    total_acertos = RespostaQuestao.objects.filter(usuario=usuario, acertou=True).count()
    ...
    desempenho_por_assunto = RespostaQuestao.objects.filter(
